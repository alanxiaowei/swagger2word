...
public class WordController {
	...
	public String getWord(Model model, @ApiParam(value = "资源地址", required = false) @RequestParam(value = "url", required = false) String url, @ApiParam(value = "是否下载", required = false) @RequestParam(value = "download", required = false, defaultValue = "1") Integer download) {
	...
	private void writeContentToResponse(Model model, HttpServletResponse response) {
		...
		Map<String, Object> contextMap = parseApiResponseProperties(model);
	...
	private Map<String, Object> parseApiResponseProperties(Model model) {
		Map<String, Object> map = model.asMap();
		System.out.println(map.toString());
		TreeMap treeMap = (TreeMap) map.get("tableMap");

		treeMap.forEach((k, v) -> {
			List tmpList = (List) v;
			tmpList.forEach(x -> {
				Table table = (Table) x;
				if (StringUtils.isEmpty(table.getResponseParam())) {
					table.setResponseParam("{\"code\":\"200\",\"msg\":\"success\",\"data\":object}");
				} else {
					StringBuffer stringBuffer = new StringBuffer();
					stringBuffer.append("{\"code\":\"200\",\"msg\":\"success\",\"data\":\"");
					stringBuffer.append(table.getResponseParam());
					stringBuffer.append("\"}");
					table.setResponseParam(stringBuffer.toString());
				}

				List rspProperties = table.getModelAttr().getProperties();
				List newRspProperties = new ArrayList();

				ModelAttr modelAttrCode = new ModelAttr();
				modelAttrCode.setRequire(true);
				modelAttrCode.setName("code");
				modelAttrCode.setType("string");
				modelAttrCode.setDescription("应答码");
				modelAttrCode.setCompleted(false);
				newRspProperties.add(modelAttrCode);

				ModelAttr modelAttrMsg = new ModelAttr();
				modelAttrMsg.setRequire(true);
				modelAttrMsg.setName("msg");
				modelAttrMsg.setType("string");
				modelAttrMsg.setDescription("应答信息");
				modelAttrMsg.setCompleted(false);
				newRspProperties.add(modelAttrMsg);
				...
				if (!ListUtils.isEmpty(rspProperties)) {
					modelAttrData.setProperties(rspProperties);
				}
				newRspProperties.add(modelAttrData);
				table.getModelAttr().setProperties(newRspProperties);
			});
		});
		return map;
	}
